...
function initCoursePlanner() {
    ...
    if (form) {
        form.addEventListener("submit", (e) => {
            ...
            const dayVal = parseInt(document.getElementById("course-day").value) || 0;
            const periodsVal = document.getElementById("course-periods").value;
            let periods = [];
            if (dayVal > 0 && periodsVal) {
                const parts = periodsVal.split("-").map(Number);
                if (parts.length === 2) {
                    for (let i = parts[0]; i <= parts[1]; i++) {
                        periods.push(i);
                    }
                } else if (parts.length === 3) {
                    for (let i = parts[0]; i <= parts[2]; i++) {
                        periods.push(i);
                    }
                } else {
                    periods = parts;
            ...
            // 檢查上課時間衝突
            if (dayVal > 0 && periods.length > 0) {
                const conflict = currentCourses.find(c => 
                    c.code.toLowerCase() !== code.toLowerCase() && 
                    c.day === dayVal && 
                    c.periods && c.periods.some(p => periods.includes(p))
                );
                if (conflict) {
                    showToast("時段衝突警告", `與「${conflict.name}」上課時段重疊！`, "warning");
                }
            }
            ...
            if (existIndex >= 0) {
                currentCourses[existIndex] = { 
                    code, name, credits, type, score,
                    day: dayVal,
                    periods: periods
                };
                showToast("更新成功", `課程「${name}」資訊已更新！`, "success");
            } else {
                currentCourses.push({ 
                    code, name, credits, type, score,
                    day: dayVal,
            ...
            document.getElementById("course-periods").value = "";
...
function renderWeeklyTimetable() {
    ...
    const unscheduledContainer = document.getElementById("unscheduled-courses-container");
    const unscheduledList = document.getElementById("unscheduled-courses-list");
    const unscheduledCount = document.getElementById("unscheduled-count");
    
    if (!grid || !activeStudent) return;
    
    grid.innerHTML = "";
    
    // 1. 渲染課表表頭
    const days = ["時間", "星期一", "星期二", "星期三", "星期四", "星期五"];
    days.forEach(day => {
        const cell = document.createElement("div");
        cell.className = "schedule-header-cell";
        cell.innerText = day;
    ...
    const timeSlots = [
        ...
        { id: 5, label: "第 5 節", time: "12:10-13:00" },
        { id: 6, label: "第 6 節", time: "13:10-14:00" },
        { id: 7, label: "第 7 節", time: "14:10-15:00" },
        { id: 8, label: "第 8 節", time: "15:10-16:00" },
        { id: 9, label: "第 9 節", time: "16:10-17:00" },
        { id: 10, label: "第 10 節", time: "17:10-18:00" }
    ];
    
    // 預先鋪滿空白網格以方便點擊定位
    for (let periodNum = 1; periodNum <= 10; periodNum++) {
        const timeSlot = timeSlots[periodNum - 1];
        const timeCell = document.createElement("div");
        timeCell.className = "schedule-time-cell";
        timeCell.style.gridColumn = "1";
        timeCell.style.gridRow = `${periodNum + 1}`;
        timeCell.innerHTML = `<strong>${timeSlot.label}</strong><span>${timeSlot.time}</span>`;
        grid.appendChild(timeCell);
        
        for (let dayNum = 1; dayNum <= 5; dayNum++) {
            const emptyCell = document.createElement("div");
            emptyCell.className = "schedule-empty-cell";
            emptyCell.style.gridColumn = `${dayNum + 1}`;
            emptyCell.style.gridRow = `${periodNum + 1}`;
            emptyCell.title = `點擊快速在 週${["一","二","三","四","五"][dayNum-1]} 第 ${periodNum} 節 排課`;
            emptyCell.addEventListener("click", () => {
                quickSelectSlot(dayNum, periodNum);
            });
    ...
    courses.forEach(course => {
        if (course.day && course.periods && course.periods.length > 0) {
            ...
            let typeZh = "必修";
            let themeClass = "type-required";
            if (course.type === "elective") {
                typeZh = "選修";
                themeClass = "type-elective";
            } else if (course.type === "general") {
                typeZh = "通識";
                themeClass = "type-general";
            }
            
            card.className = `timetable-course-card ${themeClass}`;
            ...
            card.innerHTML = `
                <div class="course-card-inner">
                    ...
                    <strong class="course-name" title="${course.name}">${course.name}</strong>
                    <div class="course-meta">
                        <span class="course-credits">${course.credits} 學分</span>
                        <span class="course-type-badge">${typeZh}</span>
                    </div>
                    <button class="course-delete-btn" onclick="unassignCourseTime('${course.code}')" title="移除此課程的時間分配" type="button">
    ...
    if (unscheduled.length > 0) {
        ...
        unscheduledCount.innerText = unscheduled.length;
        ...
        unscheduled.forEach(course => {
            ...
            let typeZh = "必修";
            let themeClass = "badge-success";
            if (course.type === "elective") {
                typeZh = "選修";
                themeClass = "badge-primary";
            } else if (course.type === "general") {
                typeZh = "通識";
                themeClass = "badge-warning";
            }
            
            item.innerHTML = `
                <span class="badge ${themeClass}">${course.code}</span>
                <strong>${course.name}</strong> (${course.credits}學分)
                <button class="btn btn-secondary btn-sm" onclick="quickScheduleSingle('${course.code}')" title="自動安排此課程時間" type="button" style="padding: 2px 6px; font-size: 11px;">
                    <i class="fa-solid fa-clock"></i> 自動排
                </button>
            `;
...
// 移除課程時間分配
window.unassignCourseTime = function(code) {
    const course = activeStudent.currentCourses.find(c => c.code === code);
    if (course) {
        course.day = 0;
        course.periods = [];
        saveActiveStudent();
        showToast("時段移除", `已將「${course.name}」從課表中退回！`, "info");
    }
};

// 點擊空白格線快速跳轉排課
window.quickSelectSlot = function(dayNum, periodNum) {
    document.getElementById("course-day").value = dayNum;
    
    const credits = parseInt(document.getElementById("course-credits").value) || 3;
    let periodsVal = `${periodNum}-${periodNum + credits - 1}`;
    
    if (periodNum + credits - 1 > 10) {
        periodsVal = `${11 - credits}-10`; // fallback to fit within 10
    }
    
    const periodsSelect = document.getElementById("course-periods");
    let found = false;
    for (let opt of periodsSelect.options) {
        if (opt.value === periodsVal || opt.value.startsWith(`${periodNum}-`)) {
            periodsSelect.value = opt.value;
            found = true;
            break;
        }
    }
    if (!found) {
        periodsSelect.value = "3-4"; // safe default
    }
    
    navigateToTab("course-planner");
    document.getElementById("course-code").focus();
    showToast("快速排課", `已預選 週${["一","二","三","四","五"][dayNum-1]} 第 ${periodNum} 節開始的上課時段！`, "success");
};

// 一鍵為所有未排課課程自動生成時段
window.autoGenerateSchedule = function() {
    if (!activeStudent || !activeStudent.currentCourses) return;
    
    const courses = activeStudent.currentCourses;
    const occupied = Array(5).fill(null).map(() => Array(10).fill(false));
    
    // 標記已被佔用的時段
    courses.forEach(c => {
        if (c.day && c.periods && c.periods.length > 0) {
            c.periods.forEach(p => {
                if (c.day >= 1 && c.day <= 5 && p >= 1 && p <= 10) {
                    occupied[c.day - 1][p - 1] = true;
                }
            });
        }
    });
    ...
    courses.forEach(c => {
        if (!c.day || !c.periods || c.periods.length === 0) {
            ...
            const duration = Math.min(4, Math.max(1, credits));
            let found = false;
            
            // 搜尋順序：週三、週二、週四、週一、週五
            const daySearchOrder = [3, 2, 4, 1, 5]; 
            // 搜尋起點節次：第 2 節、第 6 節、第 3 節、第 7 節、第 1 節、第 5 節
            const startPeriodSearchOrder = [2, 6, 3, 7, 1, 5, 8]; 
            
            for (let day of daySearchOrder) {
                const dIdx = day - 1;
                for (let p of startPeriodSearchOrder) {
                    const pIdx = p - 1;
                    if (pIdx + duration <= 10) {
                        let free = true;
                        for (let k = 0; k < duration; k++) {
                            if (occupied[dIdx][pIdx + k]) {
                                free = false;
                                break;
                            }
                        }
            ...
            // 如果上述最佳時段找不到，則順序掃描所有剩餘空間
            if (!found) {
                for (let d = 1; d <= 5; d++) {
                    const dIdx = d - 1;
                    for (let p = 1; p <= 10 - duration + 1; p++) {
                        const pIdx = p - 1;
                        let free = true;
                        for (let k = 0; k < duration; k++) {
                            if (occupied[dIdx][pIdx + k]) {
    ...
    if (scheduledCount > 0) {
        saveActiveStudent();
        showToast("自動生成成功", `已自動為 ${scheduledCount} 門課程分配無衝突上課時段！`, "success");
    } else {
        showToast("無須自動排課", "所有課程皆已排定上課時段，或課表已滿無法排入！", "warning");
    }
};

// 單一課程自動排課
window.quickScheduleSingle = function(code) {
    const course = activeStudent.currentCourses.find(c => c.code === code);
    if (!course) return;
    
    const occupied = Array(5).fill(null).map(() => Array(10).fill(false));
    activeStudent.currentCourses.forEach(c => {
        if (c.code !== code && c.day && c.periods && c.periods.length > 0) {
            c.periods.forEach(p => {
                if (c.day >= 1 && c.day <= 5 && p >= 1 && p <= 10) {
                    occupied[c.day - 1][p - 1] = true;
                }
    ...
    if (found) {
        saveActiveStudent();
        showToast("排課成功", `已自動為「${course.name}」分配合適時段！`, "success");
    } else {
        showToast("無可用時段", `課表太擠，找不到連續的 ${duration} 節空堂來排「${course.name}」！`, "error");
    }
};
...
window.clearSchedule = function() {
    ...
    if (confirm("您確定要清除本學期所有課程的上課時段設定嗎？（這不會刪除課程，僅清空時間）")) {
        activeStudent.currentCourses.forEach(c => {
            c.day = 0;
            c.periods = [];
        });
        saveActiveStudent();
        showToast("課表清除", "已清除所有上課時間設定！課程已退回未排課區域。", "warning");
    }
};
